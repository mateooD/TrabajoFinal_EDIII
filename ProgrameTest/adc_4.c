...
void TIMER0_IRQHandler(void) {
	...
	if (TIM_GetIntStatus(LPC_TIM0, TIM_MR0_INT) == SET) {
	    ...
	    uint16_t adcValue = ADC_ChannelGetData(LPC_ADC, ADC_CHANNEL_5); // Cambio a canal 5        adcBuffer[bufferIndex] = adcValue;
...
void TIMER0_IRQHandler(void) {
    if (TIM_GetIntStatus(LPC_TIM0, TIM_MR0_INT) == SET) {
        ...
        adcBuffer[bufferIndex] = ADC_ChannelGetData(LPC_ADC, ADC_CHANNEL_2);
        bufferIndex = (bufferIndex + 1) % BUFFER_SIZE;
